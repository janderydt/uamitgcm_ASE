...
Lon = Lon(:); Lat = Lat(:);
%PIG
plot([Lon(58) Lon(58) Lon(110) Lon(110) Lon(58)]+0.16e6,[Lat(310) Lat(end-20) Lat(end-20) Lat(310) Lat(310)],":","color",osfcolor,"linewidth",3);
%TW
plot([Lon(110) Lon(110) Lon(147) Lon(147) Lon(110)]+0.16e6,[Lat(150) Lat(260) Lat(260) Lat(150) Lat(150)],":","color",osfcolor,"linewidth",3);
%CR
plot([Lon(130) Lon(130) Lon(190) Lon(190) Lon(130)]+0.16e6,[Lat(35) Lat(110) Lat(110) Lat(35) Lat(35)],":","color",osfcolor,"linewidth",3);
%DT
plot([Lon(140) Lon(140) Lon(190) Lon(190) Lon(140)]+0.16e6,[Lat(5) Lat(33) Lat(33) Lat(5) Lat(5)],":","color",osfcolor,"linewidth",3);
...
for gg=1:numel(basins)
    ...
    h = plot(ax1(gg),time(I),[bsf.(basin).moving(I).amp]/1e6,"-","color",bsfcolor,"linewidth",1);
    ...
    g(1)=plot(ax2(gg),time(I),[osf.(basin).amp(I)]/1e6,"-","color",osfcolor,"linewidth",1);
    %g(2)=plot(ax2(gg),time(I),,"-","color",bsfcolor2,"linewidth",1);
    ...
    tmp = melt.(basin);
    [R3,P3] = corrcoef(tmp(I),[bsf.(basin).moving(I).amp],'rows','complete');
    fprintf('Basin: %s - Correlation between melt and barotropic streamfunction amplitude: %d \n',basin,R3(1,2));

    [R4,P4] = corrcoef(tmp(I),integral2D.(basin).monthly.btVEL_mean(I),'rows','complete');
    fprintf('Basin: %s - Correlation between melt and average depth-mean flow: %d \n',basin,R4(1,2));
    ...
    if gg==1
        ...
        %text(ax1(gg),0.95,0.05,['R = ',num2str(R3(1,2),2)],Units='normalized',HorizontalAlignment='right',VerticalAlignment='bottom',Margin=5,EdgeColor='k',BackgroundColor='w');
    ...
    elseif gg==2
        ...
        ax3(gg) = axes('position', ax2(gg).Position+[0.06 0 0 0]); hold on;
        ax3(gg).XAxis.Color='none';
        yyaxis(ax3(gg),'right');
        ax3(gg).Color = 'none';
        grid(ax3(gg), 'off');
        ax3(gg).XLim = ax2(gg).XLim;
        ax3(gg).XTick = ax2(gg).XTick;
        ax3(gg).YAxis(1).Color='none';
        ax3(gg).YAxis(2).Color=osfcolor;
        grid(ax3(gg),'off');
    ...
    elseif gg==3
        ...
        yticklabels(ax1(gg),{});
        ylim(ax1(gg),[0 1]);
        yticks(ax1(gg),[0:0.2:1]);
        
        yyaxis(ax2(gg),'right');
        yticklabels(ax2(gg),{});
        ylim(ax2(gg),[0 0.5]);

        xlabel(ax1(gg),"time [yr]");
    ...
    elseif gg==4
        ...
        ax3(gg).YAxis(2).Color=osfcolor;
        grid(ax3(gg),'off');
        ylabel(ax3(gg),"Overturning streamfunction amplitude [Sv]");
        ...
        ylim(ax1(gg),[0 1]);
        yticks(ax1(gg),[0:0.2:1]);
        ylabel(ax1(gg),"Barotropic streamfunction amplitude [Sv]");
